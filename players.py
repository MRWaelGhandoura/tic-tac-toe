...
class NoobComputerPlayer(ActualPlayer):
    ...
    It makes its moves randomly, depending on what
    moves are available on the game board.
...
class ProComputerPlayer(ActualPlayer):
    ...
    def minimax(self, state, player):
        ...
        if state.current_winner == other_player:
            return {'position': None, 'score': 1 * (state.num_empty_squares() + 1) if other_player == max_player else -1 * (  # noqa
